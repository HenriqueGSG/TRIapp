...
import {
  validateDay,
  validateDuplicateFunciId,
  validateListFunciLength
...
export class CalendarService {
  ...
  async createCalendar(month: number): Promise<Calendar> {
    const existingCalendar = await this.calendarModel
      .findOne({ monthIntRef: month })
      .exec();
  ...
  async removeFunciFromDay(
    ...
    selectedDayObject.listFunci = selectedDayObject.listFunci.filter(
      (id) => id !== funciId
    );
    daySelected.markModified('days');
    await daySelected.save();
